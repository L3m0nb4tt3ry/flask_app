...
@app.route('/userHome')
def showUserHome():
    # check that someone has logged in correcfmaintly
    if session.get("user"):
...
def validateLogin():
    try:
        ...
        if len(data) > 0:
            print("Hashed password from database:", str(data[0][3]))
            print("Password from user:", _password)
            # This returns the only user ID print("Debug this value:", str(data[0][0])) [0] returns whole row
...
def addWish():
    try:
        if session.get('user'):
            ...
            _user = session.get('user')[0]  # Fixed to have only user
